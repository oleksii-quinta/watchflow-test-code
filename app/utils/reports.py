...
def get_revenue_summary(start_date: datetime, end_date: datetime, currency: str = "USD") -> dict:
    ...
    sql = f"""
        SELECT
            SUM(CASE WHEN payment_type = 'charge' AND status = 'succeeded' THEN amount_cents ELSE 0 END) AS gross,
            SUM(CASE WHEN payment_type = 'refund' THEN amount_cents ELSE 0 END) AS refunds,
            COUNT(CASE WHEN payment_type = 'charge' AND status = 'succeeded' THEN 1 END) AS charge_count
        FROM payments
...
def get_user_search(query_string: str) -> list:
    ...
    See issue #389.
    """
    sql = f"SELECT id, email, username, role FROM users WHERE email LIKE '%{query_string}%' OR username LIKE '%{query_string}%' LIMIT 50"
